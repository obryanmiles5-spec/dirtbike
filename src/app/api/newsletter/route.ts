...
import { getZohoTransporter, getSenderAddress } from '@/lib/zohoMail';
...
export async function POST(request: Request) {
  try {
    ...
    const userMailOptions = {
      from: `"VOLT-X Off-Road Alerts" <${senderEmail}>`,
      to: email,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; background-color: #080c14; color: #ffffff; padding: 28px; border-radius: 12px; border: 2px solid #a3e635;">
          ...
          <div style="background-color: #0f172a; padding: 14px; border-radius: 8px; border: 1px border-zinc-800; margin-top: 20px; text-align: center;">
    ...
    const adminMailOptions = {
      from: `"VOLT-X Newsletter" <${senderEmail}>`,
      to: senderEmail,
    ...
    transporter.sendMail(adminMailOptions).catch(err => {
      console.warn('Admin newsletter alert failed:', err);
    });

    return NextResponse.json({
      success: true,
      message: 'Subscribed and confirmation email sent via Zoho Mail.',
      timestamp: new Date().toISOString()
    });
  } catch (error: any) {
    console.error('[VOLT-X Zoho Newsletter Error]', error);
    return NextResponse.json(
