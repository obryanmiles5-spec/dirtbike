...
export async function POST(request: Request) {
  try {
    ...
    const adminMailOptions = {
      from: `"VOLT-X Motorsports Web" <${senderEmail}>`,
      to: senderEmail,
    ...
    if (email && email.includes('@')) {
      const userMailOptions = {
        from: `"VOLT-X Motorsports Direct" <${senderEmail}>`,
        to: email,
      ...
      // Fire auto responder asynchronously
      transporter.sendMail(userMailOptions).catch(err => {
        console.warn('Auto responder email failed:', err);
      });
    }

    return NextResponse.json({
      success: true,
      message: 'Direct dispatch sent via Zoho Mail.',
      timestamp: new Date().toISOString()
    });
  } catch (error: any) {
    console.error('[VOLT-X Zoho Contact API Error]', error);
    return NextResponse.json(
      { success: false, error: error?.message || 'Failed to send mail via Zoho' },
      { status: 500 }
    );
  }
}
