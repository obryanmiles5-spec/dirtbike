  ...
  try {
    ...
    if (customerEmail && customerEmail.includes('@')) {
      const customerMailOptions = {
        from: `"VOLT-X Motorsports Orders" <${senderEmail}>`,
        to: customerEmail,
    ...
    // 2. Send Admin Notification to contact@www.voltdirtbike.com
    const adminMailOptions = {
      from: `"VOLT-X Store Orders" <${senderEmail}>`,
      to: senderEmail,
    ...
    transporter.sendMail(adminMailOptions).catch(err => {
      console.warn('Admin order mail alert failed:', err);
    });

    return NextResponse.json({
      success: true,
      orderId,
      message: 'Order recorded and confirmation emailed via Zoho Mail.',
      timestamp: new Date().toISOString()
    });
  } catch (error: any) {
    console.error('[VOLT-X Zoho Order API Error]', error);
    return NextResponse.json(
