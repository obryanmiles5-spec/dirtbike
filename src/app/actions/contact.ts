...
export async function submitContactAction(
  ...
  const user = (process.env.ZOHO_MAIL_USER || 'contact@voltdirtbike.com').replace('@www.', '@');
  ...
  try {
    ...
    const userMailOptions = {
      from: `"VOLT-X Motorsports Direct" <${user}>`,
      to: email,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; background-color: #090d16; color: #ffffff; padding: 28px; border-radius: 12px; border: 2px solid #a3e635;">
          ...
          <p style="font-size: 13px; color: #94a3b8;">Need immediate phone support or technical assistance? Call our factory hotline at <strong>505-652-1743</strong> or email <strong>${user}</strong>.</p>
    ...
    console.error('[VOLT-X Server Action Contact Error]', error);
    return {
      success: false,
      error: error?.message || 'Failed to send email. Please check your network or try again.'
    };
  }
}
